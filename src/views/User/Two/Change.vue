  ...
  <div>
    ...
    <div class="card">
      ...
      <div class="bottom">
        ...
        <div class="pushBtn" @click="submitClick">完成</div>
      </div>
...
export default {
  ...
  created() {
    let item = JSON.parse(localStorage.getItem('ji'));
    console.log(item)
    this.id=item.id
    this.curVarieties = item.varieties
    this.curSpecies = item.species
    this.curAge = item.age
    this.curSex = item.sex
    this.contentStr = item.content
    this.startTime = item.startTime
    this.endTime = item.endTime
    for (let v of this.vaccineData) {
      if (item.vaccine.indexOf(v.name) !== -1) {
        v.check = true
      }
    }
  },
  ...
  methods: {
    submitClick() {
      ...
      request.post("/upPic", formData).then(res => {
        request.post('/user/updateAdoption', {
