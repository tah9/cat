<template>
  <div>
    ...
    <div v-if="item.state===0" style="display: flex;justify-content: space-evenly;margin-top: 3em">
      <van-button @click="tong" round type="primary">审核通过</van-button>
      <van-button @click="wu" round type="warning">信息有误</van-button>
    </div>
  </div>
</template>
...
export default {
  ...
  created() {
    this.item = JSON.parse(localStorage.getItem('shenJi'))
